...
export const getOriginalRanking = async (): Promise<IllustEntry[]> => {
    ...
    const responses = await Promise.all([ axios.get(`${URL}&p=1`), axios.get(`${URL}&p=2`), axios.get(`${URL}&p=3`) ])

    return responses
        .filter(res => res.status == 200)
        .map(res => res.data.contents.map((content): IllustEntry => ({
...
export const getRanking = async (content: 'illust'|'manga'|'ugoira'): Promise<IllustEntry[]> => {
